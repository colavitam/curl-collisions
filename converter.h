// from https://github.com/iotaledger/ccurl/blob/master/src/lib/util/converter.c
...
void trits_from_trytes(const char* trytes, int length, char* trits) {
  int i;
  for (i = 0; i < length; i++) {
...
void trytes_from_trits(char* const trits, const int size, char* trytes) {
  int i;
  const int length =
...
void init_converter() {
  int i;
  char trits[TRITS_IN_BYTE];
  memset(trits, 0, TRITS_IN_BYTE * sizeof(char));
